...
# build_img.sh filename.qcow2 size distro
...
if [ -f "$FILE" ]; then
	echo "[1/1] File already exists, error"
	exit 1
fi

echo '[1/10] Partitioning virtual disk'
/usr/bin/qemu-img create -q -f qcow2 -o compat=1.1,cluster_size=2048 "$FILE" "$CFG_SIZE"
...
for foo in `seq 0 9`; do
	set +e
	DEVICE="/dev/nbd"$foo""
	qemu-nbd -c "${DEVICE}" -f qcow2 "$FILE" >/dev/null 2>&1
	RES="$?"
	set -e
	if [ "$RES" = 0 ]; then
		TARGET="/mnt/target${foo}"
		break
	fi
done
if [ "$TARGET" = "" ]; then
	echo "[2/2] System busy; please try again later"
	exit 1
fi
...
cat /proc/mounts | grep "${TARGET}" | awk '{ print $2 }' | sort -r | xargs umount
sync
